                                                        ...
                                                        const AString & pchSourceFile )
{
    ...
    // Check PchDataCache for already-loaded metadata (includes undef block if populated)
    uint64_t pchId;
    ...
    // Generate undef block if not already cached
    if ( undefBlock.IsEmpty() )
    {
        if ( GetUndefBlock( compilerExe, compilerEnvironment, pchCreationOptions, pchSourceFile, pchId, undefBlock ) == false )
        {
            FLOG_ERROR( "PCH distribution: failed to generate undef block for '%s' (pchId 0x%016" PRIx64 ")\n",
                        pchFilePath.Get(), pchId );
            return false;
        }
        PchDataCache::Get().SetUndefBlock( pchId, undefBlock );
    }
    ...
    const size_t headerSize = sizeof( uint32_t ) + sizeof( uint64_t ); // magic + pchId
    ...
    *dest = '\0';
    ...
    FLOG_VERBOSE( "PCH distribution: '%s' stripped %.1f KB -> %.1f KB (%.1f%% reduction, undef block %.1f KB)\n",
                  job->GetNode()->GetName().Get(),
...
// GetUndefBlock
//------------------------------------------------------------------------------
/*static*/ bool PchDistribution::GetUndefBlock( const AString & compilerExe,
                                                const char * compilerEnvironment,
                                                const AString & pchCreationOptions,
                                                const AString & pchSourceFile,
                                                uint64_t pchId,
...
{
    // Serialize /PD passes — multiple build threads may call this for the
    // same PCH simultaneously. The first thread runs the pass; subsequent
    // threads find the result in the disk cache or PchDataCache.
    static Mutex s_PdMutex;
    MutexHolder mh( s_PdMutex );

    // Re-check in-memory cache under the lock (another thread may have populated it)
    {
        PchDataCache::Entry cached;
        if ( PchDataCache::Get().Find( pchId, cached ) && !cached.undefBlock.IsEmpty() )
        {
            outUndefBlock = cached.undefBlock;
            return true;
        }
    }

    // Check disk cache
    if ( LoadUndefBlockFromDisk( pchId, outUndefBlock ) )
    {
        FLOG_VERBOSE( "PCH undef: loaded from disk cache (pchId 0x%016" PRIx64 ", %u bytes)\n",
                      pchId, outUndefBlock.GetLength() );
        return true;
    }

    // Run the /PD pass
    if ( RunMacroDumpPass( compilerExe, compilerEnvironment, pchCreationOptions, pchSourceFile, pchId, outUndefBlock ) == false )
    {
        return false;
    }

    // Save to disk cache for future builds
    SaveUndefBlockToDisk( pchId, outUndefBlock );

    FLOG_VERBOSE( "PCH undef: generated from /PD pass (pchId 0x%016" PRIx64 ", %u bytes)\n",
                  pchId, outUndefBlock.GetLength() );
    return true;
}
...
// RunMacroDumpPass
//------------------------------------------------------------------------------
/*static*/ bool PchDistribution::RunMacroDumpPass( const AString & compilerExe,
                                                   const char * compilerEnvironment,
                                                   const AString & pchCreationOptions,
                                                   const AString & pchSourceFile,
                                                   uint64_t pchId,
                                                   AString & outUndefBlock )
{
    ...
    // This avoids compilation (which may fail under /Zc:preprocessor for non-
    // conformant code) and avoids huge stdout (preprocessed output goes to file).
    StackArray<AString> tokens;
    ...
    for ( size_t i = 0; i < tokens.GetSize(); ++i )
    {
        ...
        // Strip /Fp (no PCH file needed)
        if ( ObjectNode::IsStartOfCompilerArg_MSVC( token, "Fp" ) )
        {
            continue;
        }

        // Strip /Fo (no obj output)
        ...
        // These appear standalone (%1) or embedded in flags (/Fo"%2").
        {
    ...
    // Direct the .i output to a temp directory
    AStackString tmpDir;
    if ( FBuild::GetTempDir( tmpDir ) == false )
    {
        return false;
    }
    #if defined( __WINDOWS__ )
        tmpDir += ".fbuild.tmp\\pch_pd\\";
    #else
        tmpDir += "_fbuild.tmp/pch_pd/";
    #endif
    FileIO::EnsurePathExists( tmpDir );

    // Use pchId in filename to avoid parallel write conflicts when multiple
    // PCHs share the same source filename (e.g. precompiled.cpp).
    // Specify full filename to avoid trailing-backslash quoting issue
    // with /Fi"dir\" where \" is parsed as an escaped quote.
    AStackString iFile( tmpDir );
    iFile.AppendFormat( "%016" PRIx64 ".i", pchId );
    ...
    p.WaitForExit();
    // Exit code may be non-zero when /Zc:preprocessor rejects non-conformant
    // macro usage. The /PD #define lines are still written to the .i file up
    // to the point of failure, which is sufficient (PCH macros are defined
    // early in the header). We check for content in the .i file below.

    // Read the .i file which contains #define lines interleaved with preprocessed output
    FileStream f;
    ...
    // Verify we got at least some #define output
    if ( iContent.Find( "#define " ) == nullptr )
    {
        FLOG_ERROR( "PCH /PD pass: no macro definitions in '%s'\nstderr:\n%s\n",
                    iFile.Get(), stdErr.Get() );
        return false;
    }

    ParseMacroDumpOutput( iContent, outUndefBlock );
    return true;
}
...
// LoadUndefBlockFromDisk
//------------------------------------------------------------------------------
/*static*/ bool PchDistribution::LoadUndefBlockFromDisk( uint64_t pchId, AString & outBlock )
{
    AStackString path;
    if ( FBuild::GetTempDir( path ) == false )
    {
        return false;
    }
    #if defined( __WINDOWS__ )
        path.AppendFormat( ".fbuild.tmp\\pch_undefs\\%016" PRIx64 ".txt", pchId );
    #else
        path.AppendFormat( "_fbuild.tmp/pch_undefs/%016" PRIx64 ".txt", pchId );
    #endif

    FileStream f;
    if ( f.Open( path.Get(), FileStream::READ_ONLY ) == false )
    {
        return false;
    }
    ...
    outBlock.SetLength( (uint32_t)fileSize );
    if ( f.Read( outBlock.Get(), fileSize ) != fileSize )
    {
        outBlock.Clear();
        return false;
    }
...
// SaveUndefBlockToDisk
//------------------------------------------------------------------------------
/*static*/ void PchDistribution::SaveUndefBlockToDisk( uint64_t pchId, const AString & block )
{
    AStackString dir;
    if ( FBuild::GetTempDir( dir ) == false )
    {
        return;
    }
    #if defined( __WINDOWS__ )
        dir += ".fbuild.tmp\\pch_undefs\\";
    #else
        dir += "_fbuild.tmp/pch_undefs/";
    #endif
    FileIO::EnsurePathExists( dir );

    AStackString path( dir );
    path.AppendFormat( "%016" PRIx64 ".txt", pchId );

    FileStream f;
    if ( f.Open( path.Get(), FileStream::WRITE_ONLY ) )
    {
        f.Write( block.Get(), block.GetLength() );
    }
}
